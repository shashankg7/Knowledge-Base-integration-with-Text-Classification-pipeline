...
def doc2vec300(text, k):
	return doc2vec300_model.infer_vector(tokenize(text, k), alpha=0.025, min_alpha=0.025, steps=5)


def word2vec(text, k):
	tokens = tokenize(text, k)
        n = len(tokens)
        n = n/k
        tokens = tokens[:n]
	#if len(tokens) == 0:
